      ...
      <div className="container mx-auto px-8 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-8">
          <div className="flex items-center gap-8">
            {navLinks.map((link) => (
              <Link 
                key={link.name} 
                href={link.href} 
                className={`text-[10px] font-bold uppercase tracking-widest hover:text-rose-pink transition-colors flex items-center gap-2 ${link.name === 'Creator Studio' ? 'text-rose-pink' : 'text-foreground/80'}`}
              >
          ...
          <div className="flex items-center gap-3">
            <Button asChild variant="outline" size="sm" className="border-rose-pink/20 text-rose-pink hover:bg-rose-pink hover:text-white rounded-full px-6 h-10 text-[10px] font-bold uppercase tracking-widest transition-all">
              <a href={whatsappLink} target="_blank" rel="noopener noreferrer">
                <Users className="w-3.5 h-3.5 mr-2" /> Join Community
              </a>
            </Button>
            <Button asChild size="sm" className="bg-rose-pink text-white hover:bg-rose-pink/90 rounded-full px-6 h-10 text-[10px] font-bold uppercase tracking-widest shadow-lg shadow-rose-pink/20">
